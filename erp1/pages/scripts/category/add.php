...
if(isset($_POST['add_category'])){
    ...
    }else if(mysqli_num_rows($result)==0 && mysqli_num_rows($result_gst)>0)
    {
        ...
        if($row['gst_rate']!=$gst_rate){
            header("Location: http://".BASE_SERVER."/erp1/pages/add-category.php?q=error&status=gst");
            exit;
        }else{
            $query= "INSERT INTO category(category_name,hsn_code,created_by) VALUES('$category_name',$hsn_code,$employee_id)";
            $add_gst_query_result = mysqli_query($connection,$query);   
        ...
        header("Location: http://".BASE_SERVER."/erp1/pages/add-category.php?q=error&status=category");
        exit;
    }
    
    header("Location: http://".BASE_SERVER."/erp1/pages/add-category.php?q=success");
    exit;
}
?>